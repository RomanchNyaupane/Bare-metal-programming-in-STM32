...
void LCD_example(void) { 
    ...
    while(!(I2C1->ISR & I2C_ISR_TXIS)){} //H
    I2C1->TXDR = 0x48;
	
	  while(!(I2C1->ISR & I2C_ISR_TXIS)){} //E
    I2C1->TXDR = 0x65;
    
    while(!(I2C1->ISR & I2C_ISR_TXIS)){} //L
    I2C1->TXDR = 0x6C;
	
	  while(!(I2C1->ISR & I2C_ISR_TXIS)){}	//L
    I2C1->TXDR = 0x6C;
    
    while(!(I2C1->ISR & I2C_ISR_TXIS)){} //O
    I2C1->TXDR = 0x6F;
...
int main(){
    ...
    I2C_init();
		...
		//LCD_example();
		//uint8_t name[15] = {0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x48, 0x61, 0x72, 0x69, 0x68, 0x61, 0x72, 0x21, 0x0A};
		uint8_t name_2[15] = {0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x43, 0x68, 0x69, 0x6E, 0x74, 0x75, 0x75, 0x21, 0x0A};
		LCD_send_command(0x40);
		for(i=0;i<15;i++){
				LCD_send_data(name_2[i]);
		}
		lcd_end();
...
void lcd_end(){
		//while(!(I2C1->ISR & I2C_ISR_TC)){I2C1->TXDR = 0;}
    while(!(I2C1->ISR & I2C_ISR_STOPF)){}
    I2C1->ICR |= I2C_ICR_STOPCF;
}